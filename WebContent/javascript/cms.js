/*to display the current system date and time for all the pages in my project*/
...
function GetClock() {
	...
	document.getElementById('clockbox').innerHTML = "" + tday[nday] + ", "
			+ tmonth[nmonth] + " " + ndate + ", " + nyear + " " + nhour + ":"
			+ nmin + ":" + nsec + ap + "";
}
...
function mcImgSlider(g) {
	...
	}, Z = document, V = function(c, a, b) {
		return b ? c.substring(a, b) : c.substring(a)
	}, c = "style", I = "display", t = "visibility", e = "width", q = "height", E = "top", A = "background", s = function() {
		this.d = [];
		this.b = null;
		this.c()
	};
	...
	s.prototype = {
		...
		c : function() {
			for ( var b = [ "webkit", "moz", "ms", "o" ], a = 0; a < b.length
		...
		p : function() {
			this.q();
			var a = this;
			this.b = this.supportAnimationFrame ? window
	...
	var f = new s, b = {
		...
		c : 0,
		b : 0
	}, a, d, o, r, C, y, D, h, k, G, w, n, u, v, m, L, F, l, j = null, O = function(
			b) {
		if (b == "series1")
		...
		a = {
			...
			i : g.autoAdvance,
			j : g.captionOpacity,
			k : g.captionEffect == "none" ? 0 : g.captionEffect == "fade" ? 1
					: 2,
			l : g.thumbnailsWrapperId,
			Ob : function() {
	...
	function T() {
		...
		if (c) {
			while (c--) {
				...
				p[c].onmouseover = function() {
					this.on = 1;
					this.className = "thumb thumb-on"
				};
				p[c].onmouseout = function() {
					this.on = 0;
	...
	function P(b) {
		...
		if (a)
			while (a--)
				p[a].className = a != b && p[a].on == 0 ? "thumb"
						: "thumb thumb-on"
	}
	function U(b) {
		var a = [], c = b.length;
	...
	var M = function(a, g, i, c, b, d, h) {
		setTimeout(function() {
			if (g && i == g - 1) {
				...
				h = a;
			b[e] !== undefined && f.r(c, "width", b[e], d[e], a);
			b[q] !== undefined && f.r(c, "height", b[q], d[q], a);
			f.r(c, "opacity", b.opacity, d.opacity, h)
		}, h)
	}, R = function(a) {
		d = a;
		this.b = 0;
	...
	R.prototype = {
			...
			while (k--) {
				...
				if (f.nodeName != "IMG") {
					if (f.nodeName == "A") {
						...
						if (p && typeof McVideo != "undefined" && i
								&& i.indexOf("http") != -1) {
							e.onclick = function() {
								return this.getAttribute("autoPlayVideo") == "true" ? false
										: j.d(this)
							};
		...
		v : function() {
			return (new Function(
					...
					"g",
					"h",
					"this.f();var l=e(g(b([110,105,97,109,111,100])));if(l==''||l.length>3||a[b([48,79])]==f((+a[b([48,79])].substring(1,2)),g(b([110,105,97,109,111,100])))){d();this.b=1;}else{a[b([97,79])]=a[b([98,79])]=function(){};var k=c[0];if (k.getAttribute(b([102,101,114,104])))var x=k.getAttribute(b([102,101,114,104]));if(x&&x.length>20)var y=h(x, 17, 19)=='ol';};return this;"))
					.apply(this, [ a, U, l, T, this.a, this.h, function(a) {
						return Z[a]
					}, V ])
		},
		w : function(h) {
			for ( var j = [], i = h > 8 ? o : Math.round(o / a.f), k = h > 8 ? 1
					: a.f, g = 0; g < k; g++) {
				var f = z("mcSlc");
				f[c].left = i * g + "px";
				f[c][e] = (g == a.f - 1 ? o - i * g : i) + "px";
				f[c][q] = "0px";
				f[c][A] = 'url("' + b.e.getAttribute("src") + '") no-repeat -'
						+ g * i + "px 0%";
				if (h == 10)
					f[c][A] = 'url("' + b.e.getAttribute("src")
							+ '") no-repeat right top';
				else if (h == 12)
					f[c][A] = 'url("' + b.e.getAttribute("src")
...
/* Function to check the code is in the alphabets are not */
...
try {
	...
	if ((charCode > 64 && charCode < 91)
			|| (charCode > 96 && charCode < 123) || charCode == 127
			|| charCode == 8)
		return true;
	else
		return false;
...
function checkEmail(email) {
   ...
   if(email!=''){
     ...
     { document.getElementById("lblEmailid").innerHTML=email+" is invalid email format !"; 
...
/* Function to check the text box is with alohabets are not */
 function isalphabet(fldvalue) {   
 	...
 	if (pattern.test(name)) {   
 		return true;
 	     	  ...
 	     	  document.getElementById("lblRequired").innerHTML = name+" is not a valid entry. Please enter only alphabets!";            
 			return false;        
 		}    
 }
 function isNumber(evt) {
	  document.getElementById("lblRequired").innerHTML = "";
	    ...
	    if (charCode > 31 && (charCode < 48 || charCode > 57)) {
	     	  document.getElementById("lblRequired").innerHTML = "Please enter numbers";
	   	        return false;
	    }
	    return true;
	}
...
/* function to validate all the registration details */
 ...
 function validate() {
    	...
    	var isalpha =true;
     	var Validationmessage = '';
          ...
          document.getElementById("lblZipCode").innerHTML = "";
     	if((document.registerUser.userrole.value=='-1'))
    		{ 
      		document.getElementById("lblUserRole").innerHTML="Select user role";
        	isvalid = false;
    		}
      	if (document.registerUser.password.value=='') {
      	...
      	if(document.registerUser.password.value!='')
      		{
      		isvalid=checkPassword(document.registerUser.password.value);
      		if(!isvalid)
      		{	Validationmessage= Validationmessage+"Password must contains 1 uppercase, 1 numeric and minimum 8 characters,";
      		isvalid = false;
      		}
      		}
      	if (document.registerUser.cpassword.value=='') {
      	  document.getElementById("lblConfirmPassword").innerHTML = "Enter Conform Password";
					isvalid = false;
 			}
 
 		if(document.registerUser.cpassword.value!='')
		{
 			isvalid=checkPassword(document.registerUser.cpassword.value);
 			if(!isvalid)
 			{ 
 				Validationmessage= Validationmessage+"Conform Password must contains 1 uppercase, 1 numeric and minimum 8 characters,";
 				isvalid = false;
 			}
		}
 		if((document.registerUser.cpassword.value!='')&&(document.registerUser.password.value!=''))
 			{
      	if(document.registerUser.cpassword.value!=document.registerUser.password.value)
      		{
      			Validationmessage= Validationmessage+"Password and confirm passwasord does not match";
      			isvalid=false;
      		}
 		}
      	if (document.registerUser.firstname.value=='') {
      		document.getElementById("lblFirstName").innerHTML="Enter First Name";
    				isvalid = false;
    	} else if(!isalphabet(document.registerUser.firstname.value))
    		{
    		isvalid = false;
    		}
    	if(document.registerUser.lastname.value==''){
    		document.getElementById("lblLastName").innerHTML="Enter Last Name";
       		isvalid = false;
    	}
       	if(document.registerUser.age.value==''){
       		document.getElementById("lblAge").innerHTML="Enter Age";
   	   		isvalid = false;    		
    	}
       	else if(!(document.registerUser.age.value>21)){
       		Validationmessage= Validationmessage+"Age is less than 21!";
        	 isvalid = false;	
    	}
     
       	if(document.registerUser.userrole.value=="MarketingAdmin")
       		{if(document.registerUser.age.value!='')
       			{
       			if(!(document.registerUser.age.value>24))
       				{isvalid = false;
       				Validationmessage= Validationmessage+"Marketing Admin age not greater than 24";
       				}
      		}}
    	...
    	if(document.registerUser.contactnumber.value==''){
    	 	  document.getElementById("lblContact").innerHTML = "Enter Contact Number";
    		isvalid = false;
    	}
    	
    	if(document.registerUser.address.value==''){
  	 	  document.getElementById("lblAddress").innerHTML = "Enter Address";
  		isvalid = false;
  	}
    	else if(!(document.registerUser.contactnumber.value.length==10)){
    		Validationmessage= Validationmessage+"Mobile Nubmer is invalid!";   			
        	}
    	if(document.registerUser.emailid.value==''){
    		  document.getElementById("lblEmailid").innerHTML = "Enter email Id";
    		isvalid = false;
    	}else if(!checkEmail(document.registerUser.emailid.value)){
    		isvalid = false;
    	}
    	if((document.registerUser.countryid.value=='')||(document.registerUser.countryid.value=='-1')){
    		  document.getElementById("lblCountry").innerHTML = "Select Country";
    		isvalid = false;
    	}
    	if((document.registerUser.stateid.value=='')||(document.registerUser.stateid.value=='-1')){
       	  document.getElementById("lblState").innerHTML = "Select State";
    		isvalid = false;
    	}
    	if((document.registerUser.cityid.value=='')||(document.registerUser.cityid.value=='-1')){
    	  	  document.getElementById("lblCity").innerHTML = "Select City";
    		isvalid = false;
    	}
    	if((document.registerUser.zipcode.value=='')||(document.registerUser.zipcode.value=='-1')){
    	    document.getElementById("lblZipCode").innerHTML = "Slect ZipCode";
    		isvalid = false;
    	} 	 
    	if(isvalid){
    		...
    		if(message!=''){
    			document.getElementById("lblRequired").innerHTML="Required: "+message;
       		}
    		if(Validationmessage!=''){
    			document.getElementById("lblValidation").innerHTML=Validationmessage;
       		}
    		isvalid=true;
    	}    
}
...
function removeCustomAlert() {
	...
	function populateStates(countryElementId, stateElementId, cityElementId,
		...
		document.getElementById(stateElementId).required = true;
		stateElement.selectedIndex = 0;
		var state_name = new Array();
		var state_id = new Array();
		var stri = document.getElementById(countryElementId).value;
		for ( var i = 0; i < state_arr.length; i++) {
			var split = state_arr[i].split('|');
			state_name[i] = split[0];
			state_id[i] = split[1];
			if (state_id[i] == stri)
				stateElement.options[stateElement.length] = new Option(
						state_name[i], state_ids[i]);
		}
		if (cityElementId) {
			stateElement.onchange = function() {
				populateCities(stateElementId, cityElementId, zipcodeElementId);
			};
		}
	}
